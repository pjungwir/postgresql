# Read-write-unique test for GiST indexes

setup
{
  CREATE TABLE test (i int4range);
  CREATE UNIQUE INDEX test_i_uq ON test USING gist (i);
}
...
step w1 { INSERT INTO test VALUES ('[42,43)'); }
step c1 { COMMIT; }

session s2
setup { BEGIN ISOLATION LEVEL SERIALIZABLE; }
step r2 { SELECT * FROM test; }
step w2 { INSERT INTO test VALUES ('[42,43)'); }
step c2 { COMMIT; }

# Two SSI transactions see that there is no row with value '[42,43)'
