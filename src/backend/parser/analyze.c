							...
							bool isUpdate)
{
	 ...
	 */
	opclass = GetDefaultOpClass(ANYRANGEOID, GIST_AM_OID);
	opfamily = get_opclass_family(opclass);
	overlaps_op = get_opfamily_member(opfamily, ANYRANGEOID, ANYRANGEOID, RTOverlapStrategyNumber);

	result->overlapsExpr = (Node *) makeSimpleA_Expr(AEXPR_OP, get_opname(overlaps_op),
	...
	if (isUpdate)
	{
		...
		if (range_attno != InvalidAttrNumber)
		{
			// TODO: How to look this up without hardcoding?
			// Intersect has no amop entry or strategy number.
			// Anyway why is hardcoding wrong?
			// I guess I could move it into the exec node and call range_intersect directly.
			// But having a TLE seems important.
