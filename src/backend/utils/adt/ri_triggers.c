...
#define TRI_PLAN_SETNULL_ONDELETE		12
#define TRI_PLAN_SETNULL_ONUPDATE		13
#define TRI_PLAN_SETDEFAULT_ONDELETE	14
#define TRI_PLAN_SETDEFAULT_ONUPDATE	15
 ...
 * build_period_range -
 *
 * Build a range expression from a PERIODs start/end columns.
 */
static Datum
...
tri_set(TriggerData *trigdata, bool is_set_null, int tgkind)
{
	...
	switch (tgkind) {
		...
		case RI_TRIGTYPE_DELETE:
			...
			break;
		default:
			elog(ERROR, "invalid tgkind passed to tri_set");
	}
	...
	if ((qplan = ri_FetchPreparedPlan(&qkey)) == NULL)
	{
		...
		switch (tgkind) {
			...
			case RI_TRIGTYPE_DELETE:
				 ...
				 * the columns specified in that array, otherwise we
				 * update all the referencing columns.
				 * TODO: Make sure the range column isn't one of them.
				 */
				if (riinfo->ndelsetcols != 0) {
					num_cols_to_set = riinfo->ndelsetcols;
					set_cols = riinfo->confdelsetcols;
...
/*
 * DeconstructFkConstraintPeriod -
 *
 * Look up what we need to know about a PERIOD used in an foreign key.
 */
